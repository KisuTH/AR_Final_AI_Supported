...
var maxObstacles = 8;
var levelDistanceMeters = 350; // abstract distance units
var GAME_DURATION = 30.0; // seconds, fixed gameplay length
...
// Face lane control params
var centerLeft = 0.42; // normalized threshold
var centerRight = 0.58; // normalized threshold
var laneHysteresis = 0.04; // normalized half-width
...
var updateEvt = null;
var timeRemaining = 0; // countdown in seconds

// Face tracking lane selection state
var desiredLane = 1; // 0/1/2 based on head X
var lastHeadUpdateTime = 0; // seconds since epoch
...
// Touch controls: tap to jump, double-tap to slide during RUNNING
function setupTouchControls() {
    // Configure Touch Events block to work with in-lens gestures and not trigger camera swap
    if (script.touchEvents) {
        // Avoid interfering with UI buttons; don't block default touches globally
        if (script.touchEvents.blockDefaultTouches !== undefined) {
            script.touchEvents.blockDefaultTouches = false;
        }
        // Allow app-level double tap usage (we consume it here for slide)
        if (script.touchEvents.allowDoubleTap !== undefined) {
        ...
        // If missing Touch Events block, log a hint (prevents hard crash)
        print("TouchEvents block is missing. Add a Touch Events block with scriptName 'touchEvents'.");
    }
}
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    // Fetch textures (background now comes from Background Image block, not Sprite Store)
    ...
    // Background Image block provides environment now; keep any SpriteManager BG hidden if present
    bgSprite = script.spriteManager.createSprite("BG");
    bgSprite.visible = false;
    bgSprite.zIndex = 0; // reserved below gameplay if ever shown

    // Setup lanes and player
    initLanes();
    initOrResetPlayerSprite();
    ...
    // Only set text and safe-region to avoid styling-related runtime issues
    script.stateText.text = "Click Start to play\nMove head left/right to switch lanes";
    ...
    // Timer UI init
    script.timeBar.enabled = true; // keep RED timer bar visible
    script.timeBar.setProgress(1);
    script.timerText.enabled = true;
    script.timerText.text = "Time: " + Math.ceil(GAME_DURATION);
    ...
    script.startButton.label = "Start";
    
    // Jump/Slide buttons removed from HUD — controls moved to touch gestures

    // BLUE distance progress bar removed from HUD; ensure disabled at init above and not used during gameplay

    // Input: face-based lane control and UI buttons
    setupFaceControl();
    setupUiButtons();
    setupTouchControls();
...
// Create player and lanes (refactored to avoid duplication)
function initLanes() {
...
function initOrResetPlayerSprite() {
    if (player == null) {
...
// Face-based lane control
function setupFaceControl() {
    script.faceLandmarks.onHeadCenterPosition2DUpdated.add(function(position2d) {
        // position2d is in unit coords (0-1). We only need x.
        var x = position2d.x;
        lastHeadUpdateTime = (new Date()).getTime() * 0.001;

        var newDesired = desiredLane;
        if (desiredLane === 1) {
            if (x < centerLeft - laneHysteresis) newDesired = 0;
            else if (x > centerRight + laneHysteresis) newDesired = 2;
        } else if (desiredLane === 0) {
            if (x > centerLeft + laneHysteresis) newDesired = 1;
        } else if (desiredLane === 2) {
            if (x < centerRight - laneHysteresis) newDesired = 1;
        }

        if (newDesired !== desiredLane) {
            desiredLane = newDesired;
            if (gameState === "RUNNING") {
                beginLaneSwitch(desiredLane);
            }
        }
    });
}

// UI Buttons: Start/Restart only (Jump/Slide removed)
function setupUiButtons() {
    // Click-to-start and restart via startButton
    script.startButton.onPressEnd.add(function() {
        if (gameState === "IDLE" || gameState === "GAME_OVER" || gameState === "WIN") {
            startGame();
        }
    });
}

// Jump/Slide are exposed as functions to be callable from potential UI buttons in the future
...
function startGame() {
    ...
    script.timeBar.setProgress(1);
    script.timerText.enabled = true;
    script.timerText.text = "Time: " + Math.ceil(timeRemaining);
    ...
    // Jump/Slide buttons removed; input via touch gestures

    // BG (no-op: environment comes from Background Image block)
    // If needed at runtime, keep any SpriteManager BG hidden
...
function update() {
    ...
    if (gameState === "RUNNING") {
        // Timer countdown
        timeRemaining -= dt;
        script.timeBar.setProgress(clamp(timeRemaining / GAME_DURATION, 0, 1));
        script.timerText.text = "Time: " + Math.max(0, Math.ceil(timeRemaining));
        ...
        // Update UI (BLUE distance bar disabled; do not update it)
...
function onTimeUp() {
    // Stop motion by moving to end state like win/time up
    gameState = "GAME_OVER";
    script.stateText.enabled = true;
    script.stateText.text = "Time's up!\nClick Restart";
    script.stateText.forceSafeRegion(true);
    script.startButton.enabled = true;
    script.startButton.label = "Restart";
}
